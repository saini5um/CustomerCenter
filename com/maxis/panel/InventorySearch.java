...
//Added by Lee Wynn, Teoh (1/6/2005)
 ...
 *
 * @author  schp01
 * Edited by Lee Wynn, Teoh (1/6/2005)
 */
public class InventorySearch extends AbstractHTMLPanel implements  ContextFormListener, KeyListener {
  ...
  public InventorySearch() {
    ...
    htmlInventoryLineList.addExtraVelocityContextTopic("TemplateUtility", new TemplateUtility());
    ...
    htmlAvailableInventory.addExtraVelocityContextTopic("TemplateUtility", new TemplateUtility());
    ...
    htmlReservedInventory.addExtraVelocityContextTopic("TemplateUtility", new TemplateUtility());
    ...
    htmlAssignedInventory.addExtraVelocityContextTopic("TemplateUtility", new TemplateUtility());
  ...
  public void contextFormStateChanged(ContextFormEvent evt){
    ...
    } else if (evt.getType() == ContextFormEvent.PRE_INIT_CONTROLS) {
      // If the "Set Reservation End Date" checkbox is checked, make the ReservationEndDate attribute required
      // (for display purposes only).  Had to force this to not required when performing the search so we can
      // avoid a framework attribute validation error (framework does not like an empty required attribute):
      IContext ctx = ctxFinder.findContext();
      if(ctx != null) {
        IPersistentCollection availableInventory = (IPersistentCollection)ctx.getObject("InvElementList","Available");
        if (availableInventory != null) {
          availableInventory.getAttribute("ReservationEndDate").setRequired(availableInventory.getAttributeDataAsBoolean("UseReservationEndDate"));
        }
      }
    }
  }
  
  private boolean isReservationEndDateValid() {
    boolean retvalue = true;
    GregorianCalendar calNow = new GregorianCalendar();
    Date dteNow = calNow.getTime();
    if (edtEndDate.getDate() == null || dteNow.after(edtEndDate.getDate())) {
      retvalue = false;
    }
    return retvalue;
  }

  private boolean videoGatewayPassword(IPersistentObject invElement) {
        String password = "";
        for ( int i = 0; i < 8; i++ )
            password = password + new Integer((int)Math.round(Math.random()*9)).toString();
        
        password = com.maxis.integ.GenericUtil.encode(password);
        
        invElement.setAttributeDataAsString("ProvisioningData2", password);
        invElement.getAttribute("InventoryId").setModified(true);
        invElement.getAttribute("InventoryIdResets").setModified(true);
        invElement.flush(ModelAdapterFactory.getModelAdapter());
        
/*        String SQL = "UPDATE INVD_MAIN SET PROVISIONING_DATA_2 = ? ";
        SQL += "WHERE INVENTORY_ID = ? ";
        SQL += "AND INVENTORY_ID_RESETS = ?";
...
//                err.logErrorToFile("Error getting current Oracle Connection", getClass());
            ...
            pstmt.setLong(2, invElement.getAttributeDataAsInteger("InventoryId"));
            pstmt.setInt(3, invElement.getAttributeDataAsInteger("InventoryIdResets"));
...
*/        
    ...
    if (type.equals("paging")) {
      ...
      if (ctx != null) {
        ...
        if (type.equals("reserve") ) {
          ...
          boolean ReserveValidationErrorOccurred = false;
          ...
          if (account != null && invElement != null){
            if (availableInventory!=null){
              ...
              if (availableInventory.getAttributeDataAsBoolean("UseReservationEndDate")){
                // Only set & allow to continue if Reservation End date is greater than today:
                if (!isReservationEndDateValid()) {
                  ReserveValidationErrorOccurred = true;
                  JOptionPane.showMessageDialog(this, ResourceManager.getString("CC-4-39") );
                } else {
            ...
            if (!ReserveValidationErrorOccurred) {
        ...
        if (type.equals("assign") || type.equals("reserve-and-assign")) {
          ...
          if (service != null && invElement != null){
            ...
            if (service.addAssociation(invElement)){
              ...
              // Generate and store password for Video Gateway service
              if ( invElement.getAttributeDataAsInteger("InventoryTypeId") == 420 )
                  videoGatewayPassword(invElement);
  ...
  public boolean markItemForCancellation(IPersistentObject orderItem) {
    ...
    orderItem.getAttributeDataAsBoolean("TransientToBeCancelled") == true) {
      JOptionPane.showMessageDialog(this, ResourceManager.getString("CC-4-27"));
      return success;
    }
  ...
  private void initComponents() {//GEN-BEGIN:initComponents
    ...
    cbTopLevelContainerType.addActionListener(new java.awt.event.ActionListener() {
      public void actionPerformed(java.awt.event.ActionEvent evt) {
        cbTopLevelContainerTypeActionPerformed(evt);
      }
    });
  ...
  private void cbTopLevelContainerTypeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cbTopLevelContainerTypeActionPerformed
    
    // TODO add your handling code here:
  }//GEN-LAST:event_cbTopLevelContainerTypeActionPerformed
    ...
    private void resetControls(){
      ...
      edtAssignQuantity.initializeControl();
      chkAssignContiguousBlock.initializeControl();
      //Added by Lee Wynn, Teoh (2/6/2005)
      checkCharCount();
      //end of mod
    }
    ...
    private void btnCancelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCancelActionPerformed
      int HLR=-1;
      int objectID =-1;
      System.out.println("*** Action OK Performed");
      IContext ctx = ctxFinder.findContext();
      boolean canClose = true;
      IPersistentCollection obj = ctx.getCollection("InvElementList", "Service");
      if(obj!=null) {
        for (int i = 0;i< obj.getCount();i++) {
          IPersistentObject inventory = obj.getAt(i);
          //com.maxis.util.CommonUtil.objectToString(inventory);
          System.out.println("*** InventoryTypeId = "+inventory.getAttributeDataAsInteger("InventoryTypeId"));
          //20050906 WMH KenanFX – CC – General-110: change InventoryLineId from 1(sim) to 3 (imsi)
          //if(inventory.getAttributeDataAsInteger("InventoryLineId")==1) {//
          if(inventory.getAttributeDataAsInteger("InventoryLineId")==3) {
            System.out.println("*** ObjectID = "+objectID);
            if(objectID<0) {
              System.out.println("*** assigning vars");
              HLR = inventory.getAttributeDataAsInteger("LocationId");
              objectID = inventory.getAttributeDataAsInteger("InventoryTypeId");
            }else {
              if(HLR!=inventory.getAttributeDataAsInteger("LocationId")) {
                String[] errArr = new String[1];
                errArr[0] = "SIM Replacement has to be for the same HLR";
                IError pErr = ErrorFactory.createError("Provision Error",(Object[]) null,getClass());
                //pErr.addError(ErrorFactory.createError("ProvisionError",errArr, getClass()));
                ctxFinder.displayHTMLError(ErrorFactory.createError("ProvisionError",errArr, getClass()));
                canClose = false;
              }
            }
          }
    ...
    private void setInventoryLineId(){
      ...
      if (ctx != null) {
        ...
        if (emfConfigurationInventoryLine != null && invElementList != null){
          invElementList.resetFilterCriteria();
          ...
          System.out.println("Set selectedIndex = "+selectedIndex);
          invElementList.reset();
          invElementList.setFaulted(true);
          htmlAvailableInventory.initializeControl();
          cbInventoryTypeId.initializeControl();
        }
      }
    ...
    private void chkSetReservationEndDateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_chkSetReservationEndDateActionPerformed
      // enable/disable end date & set required-ness
      IContext ctx = ctxFinder.findContext();
      if (ctx != null) {
        IPersistentCollection invElementList = (IPersistentCollection)ctx.getObject("InvElementList", "Available");
        if (invElementList !=null) {
          invElementList.getAttribute("ReservationEndDate").setRequired(invElementList.getAttributeDataAsBoolean("UseReservationEndDate"));
          invElementList.getAttribute("ReservationEndDate").setEmpty(!invElementList.getAttributeDataAsBoolean("UseReservationEndDate"));
          edtEndDate.setEnabled(invElementList.getAttributeDataAsBoolean("UseReservationEndDate"));
    ...
    private void btnSearchActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSearchActionPerformed
      ctxFinder.clearStatusBarMessage();
      ...
      // Trick the system into thinking that this is not required when performing the search so we don't get a
      // framework validation error (can flip back on PRE_INIT_CONTROLS):
      availableInventory.getAttribute("ReservationEndDate").setRequired(false);
    ...
    /*
     *Section added by Lee Wynn, Teoh (2/6/2005)
     */
    ...
    private void checkCharCount() {
      if(selectedIndex==1) {
        if(getEdtInventoryLen()==19&&edtInventoryId.getText().indexOf("%")<0&&
        isNumber(edtInventoryId.getText())) {
    ...
    private void populateAttribute() {
      ...
      if(ctxFinder!=null) {
        ...
        System.out.println("Populated Attribute");
      }
    }
